...
The objective in this phase is to manufacture the IC and sensor for Phase 2, and test these in the lab. Our approach here is to do microfabrication at UF, and outsource the IC fabrication. The IC and sensor will be tested separately, and then together, once the functionality of each is established. The IC can be tested independently with dummy sources to verify function, and the sensor can be tested with a COTS circuit first. Varying field strength, frequency, and gradient using Helmholtz coils can establish sensitivity and noise floor of the sensor. This task will be accomplished at UF by Dr. Yoon and Dr. Joaquin Casanova. Completion of this task is specified by successful construction and testing of a sensor head, which satisfies Table \ref{table:obj}, using the circuitry developed in \ref{sec:p2:cir} and sensor in \ref{sec:p2:em}. To reduce risk of failure, fabrication will begin early in Phase 2, enabling us to revise our design to reduce manufacturing defects orenhance sensivity.
...
\begin{figure}
\centering
\includegraphics[width=0.65\textwidth]{cantifab}
...
\begin{figure}
\centering
\includegraphics[width=0.65\textwidth]{resinfab}
\caption{Molded resin deposition of NdFeB \cite{wang2013resin}. (a) Sputtering Cr/Cu seed layer; (b) Photoresist patterning; (c) Electroplating copper mold; (d) Stripping photoresist; (e) Filling the Cu mold; (f) Removing the paste residues; (g) Removing the Cu mold.}
\label{fig:resinfab}
\end{figure}
...
A few methods exist for deposition and patterning of rare earth transition metals (such as NdFeB), which have desirable high coercivity and remanent magnetization. One way is deposiion by sputtering into patterned trenches, followed by chemical etching and planarization \cite{walther2009micro}, though this technique can result in lateral overetching. Another is using high-power plasma to etch a deposited magnetic film \cite{jiang2011micro}. Yet another possible approach is to create a sacrificial mold, then fill it with a resin embedded with NdFeB (Figure \ref{fig:resinfab}). The mold is subsequently dissolved \cite{wang2013resin}. Which method we select ultimately depends on the availability of materials, the available processes at our facilities, and the compatibility of these processes with the other layers in the microfabricated structure. For instance, if the etchant damages the piezo layer, that is not a viable option.
